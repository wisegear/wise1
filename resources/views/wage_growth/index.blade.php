...
<div class="mx-auto max-w-7xl px-4 py-8 md:py-12">
            ...
            @if($previous)
                ...
                <p class="mt-1 text-sm leading-6 text-gray-700">
                    ...
                    <span class="font-semibold {{ $delta_single>0?'text-emerald-700':($delta_single<0?'text-red-700':'text-zinc-900') }}">
                        @if($delta_single>0)+{{ number_format(abs($delta_single),1) }}%@elseif($delta_single<0)-{{ number_format(abs($delta_single),1) }}%@else No change @endif
                    </span>
                </p>
                ...
                <p class="mt-1 text-sm leading-6 text-gray-700">
                    ...
                    <span class="font-semibold {{ $delta_three>0?'text-emerald-700':($delta_three<0?'text-red-700':'text-zinc-900') }}">
                        @if($delta_three>0)+{{ number_format(abs($delta_three),1) }}%@elseif($delta_three<0)-{{ number_format(abs($delta_three),1) }}%@else No change @endif
                    </span>
                </p>
            @endif
        </div>
    ...
    <section class="mb-6">
        <div class="border p-4 bg-white rounded-lg shadow">
            <div class="mb-2 text-sm font-medium text-gray-700">Wage Growth – Single Month vs 3‑Month Average</div>
            <div class="h-96"><canvas id="wageChart"></canvas></div>
        </div>
    </section>

    {{-- BONUS EFFECT EXPLANATION --}}
    <section class="mb-6">
    ...
    <section class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            ...
            @php $bonus_gap = $latest->single_month_yoy - $latest->three_month_avg_yoy; @endphp
            ...
            <table class="min-w-full text-sm">
                ...
                @forelse($all->sortByDesc('date') as $row)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border-b border-gray-100 px-4 py-2 font-medium">{{ number_format($row->single_month_yoy, 1) }}%</td>
                        <td class="border-b border-gray-100 px-4 py-2 font-medium">{{ number_format($row->three_month_avg_yoy, 1) }}%</td>
                    </tr>
                @empty
...
(function() {
    ...
    if (window._wageChart) window._wageChart.destroy();
                ...
                x: {
                    ticks: {
                        callback: function(value, index) {
                            ...
                            return raw.slice(0, 4); // Display YYYY
                        },
                        maxTicksLimit: 14
                    }
                },
