...
<div class="mx-auto max-w-7xl px-4 py-8 md:py-12">
    ...
    {{-- Hero --}}
    <section class="relative overflow-hidden rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
        <div class="max-w-5xl relative z-10">
            <div class="inline-flex items-center gap-2 rounded-full border border-zinc-300 bg-white/70 px-3 py-1 text-xs text-zinc-700 shadow-sm">
    ...
    {{-- Quick tip --}}
    <section class="mt-6 rounded-2xl border border-zinc-300 bg-orange-50 p-4 shadow-sm">
    ...
    <section class="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <a href="{{ Route::has('property.search') ? route('property.search') : url('/property/search') }}"
           class="group block rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:shadow-md">
        ...
        <a href="{{ Route::has('repossessions.index') ? route('repossessions.index') : url('/repossessions') }}"
           class="group block rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:shadow-md">
            <div class="flex items-center justify-between">
                ...
                <span class="rounded-full bg-orange-200 px-2 py-1 text-xs text-zinc-700">Beta</span>
            </div>
    ...
    <section class="mt-10 rounded-3xl border border-zinc-200 bg-white p-6">
    ...
    <div class="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-4">
        @foreach ($posts as $post)
            <a href="/blog/{{ $post->slug }}"
               class="group block rounded-3xl border border-zinc-200 bg-white p-2 shadow-sm transition hover:shadow-md">
                ...
                <div class="p-2">
                    <h2 class="mt-2 font-semibold text-zinc-900 group-hover:text-cyan-700">
                        {{ $post->title }}
                    </h2>
