...
@section('content')
    <div class="max-w-xl mx-auto bg-white p-6 rounded shadow">
        <h1 class="text-xl font-bold mb-4">Edit Postcode</h1>

        <form action="{{ route('admin.postcodes.update', $postcode->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="postcode" id="postcode"
                       value="{{ old('postcode', $postcode->postcode) }}"
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4"
                          class="mt-1 block w-full border border-gray-300 rounded px-3 py-2 focus:ring focus:ring-blue-200">{{ old('notes', $postcode->notes) }}</textarea>
