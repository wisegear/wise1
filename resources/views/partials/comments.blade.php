...
<div class="comments-section">
    ...
    @foreach($comments as $comment)
        <div class="comment my-6 text-sm border border-slate-200 rounded-lg p-5 shadow-lg bg-slate-50">
            ...
            <a class="underline text-lime-700 hover:text-lime-500" href="/profile/{{ $comment->user->name_slug }}">
                {{ $comment->user->name }}
            </a>
            said:
    ...
    <div class="mt-10">
        @can('Member')
            <form action="{{ route('comments.store') }}" method="POST">
                ...
                <textarea 
                    name="comment_text" 
                    class="w-full border border-slate-200 rounded-lg shadow-lg p-4" 
                    rows="2" 
        ...
        @guest
            <div class="mt-4">
                <p>
                    Want to comment on this page? 
                    <a href="/login" class="text-lime-700">Login</a> or 
                    <a href="/register" class="text-lime-700">Register</a>.
                </p>
            </div>
        @endguest
    </div>
