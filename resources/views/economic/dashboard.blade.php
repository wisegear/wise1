...
<div class="mx-auto max-w-7xl px-4 py-8 md:py-12">
    ...
    {{-- OVERALL STRESS SCORE PANEL --}}
    @if(!is_null($stressScore))
        @php
            $stressBarClass = $stressScore >= 70 ? 'bg-rose-200' : 
                             ($stressScore >= 40 ? 'bg-amber-200' : 'bg-emerald-100');
        @endphp
        
        <section class="mb-8 rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-5 md:p-6 shadow-lg">
            ...
            <div class="mt-4 flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
                <div class="relative h-2 w-full flex-1 rounded-full bg-white/70 border border-zinc-200 overflow-hidden">
                    <div class="h-full {{ $stressBarClass }} rounded-full transition-all duration-300" 
                         style="width: {{ max(0, min(100, $stressScore)) }}%;"></div>
                </div>
