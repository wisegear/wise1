...
<div class="mx-auto max-w-7xl px-4 py-8 md:py-12">
    ...
    <section class="relative z-0 overflow-hidden rounded-lg border border-gray-200 bg-white/80 p-6 md:p-8 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="max-w-4xl">
            ...
            <p class="mt-2 text-sm leading-6 text-gray-700">
                ...
                <span class="font-semibold">Category A sales only</span>.  This represents the widely accepted postcodes in the Outer Prime areas.  Some will refine that down
                further to specific streets or neighbourhoods, but for this site it's suitable for a broad overview.
            </p>
            <p class="mt-1 text-sm leading-6 text-gray-700">
                ...
                <span class="ml-2 text-neutral-600">|
                    ...
                    @php
                        $ts = $lastCachedAt
                            ?? \Illuminate\Support\Facades\Cache::get('outerprime:v2:catA:last_warm')
                            ?? \Illuminate\Support\Facades\Cache::get('outerprime:v1:catA:last_warm');
                    @endphp
                    @if(!empty($ts))
        ...
        <div class="mt-6 md:mt-0 md:ml-8 flex-shrink-0">
            <img src="{{ asset('assets/images/site/outer_prime.svg') }}" alt="Outer Prime London" class="w-64 h-auto">
        </div>
    </section>
    ...
    @else
        @foreach($districts as $district)
            @php $__label = ($district === 'ALL') ? 'All Outer Prime' : $district; @endphp
            <section class="mb-10 district-section" data-district="{{ $district }}">
                @if($district === 'ALL')
                    <div class="mb-4 rounded-md border border-zinc-200 bg-white p-4 text-sm text-neutral-700">
                        <h2 class="text-lg font-semibold mb-2">All Outer Prime – Overview</h2>
                        <p>This section aggregates <strong>all Outer Prime London postcodes</strong> into a single area for year-by-year analysis.</p>
                    </div>
                ...
                <div class="flex flex-col gap-3 sm:gap-4 md:grid md:grid-cols-2">
                    ...
                    <div class="rounded-lg border p-4 bg-white overflow-hidden h-56 sm:h-60 md:h-64 lg:h-72 col-span-2">
                        <h3 class="font-semibold mb-2">Top Sale Marker in {{ $__label }} (Hover over points for top 3 sales)</h3>
                        <canvas id="ts_{{ $district }}" class="w-full h-full"></canvas>
                    </div>

                    <!-- Average + Prime Indicators (line) -->
                    <div class="rounded-lg border p-4 bg-white col-span-2 overflow-hidden h-64 sm:h-72 md:h-80">
                        <h3 class="font-semibold mb-2">Average & Outer‑Prime Indicators in {{ $__label }}</h3>
...
(function(){
    ...
    function renderCharts() {
        ...
        function renderDistrict(district) {
            ...
            // Additional series (if provided by controller/warmer)
            ...
            if (scCtx) {
                ...
                new Chart(scCtx, { 
                    ...
                    options: { animation: false, responsive: true, maintainAspectRatio: false, layout: { padding: { top: 12, right: 12, bottom: 16, left: 12 } }, plugins: { legend: { display: true }, tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: ${fmtNum(ctx.parsed.y)}` } } }, scales: { y: { ticks: { callback: (v) => fmtNum(v) } } } }
                });
            ...
            if (ptCtx) {
                ...
                new Chart(ptCtx, { 
                    ...
                    options: { animation: false, responsive: true, maintainAspectRatio: false, layout: { padding: { top: 12, right: 12, bottom: 16, left: 12 } }, plugins: { legend: { display: true }, tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: ${fmtNum(ctx.parsed.y)}` } } }, scales: { x: { stacked: true }, y: { stacked: true, ticks: { callback: (v) => fmtNum(v) } } } }
                });
            ...
            if (aptCtx) {
                ...
                if (!avgPriceByType.length) {
                    ...
                    window.__upclCharts[aptId] = Chart.getChart(aptCtx);
            ...
            if (nbCtx) {
                ...
                if (!newBuildPct.length) {
                    ...
                    const note = document.createElement('p');
                    note.className = 'mt-2 text-xs text-neutral-500';
                    note.textContent = 'No new-build % series found (check cache warm / controller payload).';
                    card.appendChild(note);
                } else {
                    ...
                    window.__upclCharts[nbId] = Chart.getChart(nbCtx);
            ...
            if (durCtx) {
                ...
                if (!tenurePct.length) {
                    ...
                    const note = document.createElement('p');
                    note.className = 'mt-2 text-xs text-neutral-500';
                    note.textContent = 'No tenure % series found (check cache warm / controller payload).';
                    card.appendChild(note);
                } else {
                    ...
                    window.__upclCharts[durId] = Chart.getChart(durCtx);
